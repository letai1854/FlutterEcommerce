...
/**
 * A fallback implementation when Elasticsearch is disabled.
 * This is a standalone service rather than extending ProductElasticsearchService.
 */
@Service
...
public class FallbackProductElasticsearchService {
    ...
    public FallbackProductElasticsearchService() {
        logger.info("Using FallbackProductElasticsearchService - Elasticsearch functionality is disabled");
    }

    public void saveProduct(Product product) {
        logger.debug("Elasticsearch disabled - Not saving product with ID: {}", product != null ? product.getId() : "null");
    }

    public void deleteProduct(Long productId) {
        logger.debug("Elasticsearch disabled - Not deleting product with ID: {}", productId);
    }
    ...
    public ProductElasticsearchDTO findById(Long productId) {
        logger.debug("Elasticsearch disabled - Not finding product with ID: {}", productId);
        return null;
    }

    public Page<ProductElasticsearchDTO> searchProducts(Query searchQuery, Pageable pageable) {
        logger.debug("Elasticsearch disabled - Not searching products");
        return new PageImpl<>(Collections.emptyList(), pageable, 0);
    }

    public void updateProductEnabledStatus(Long productId, boolean isEnabled) {
        logger.debug("Elasticsearch disabled - Not updating product enabled status for ID: {}", productId);
    }

    public List<Long> searchProductIds(String search) {
        logger.debug("Elasticsearch disabled - Not searching product IDs for: {}", search);
        // If a search term is provided when ES is disabled,
